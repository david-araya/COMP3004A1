...
public class BlackJackTest extends TestCase {
	...
	/*
	 * Test that if you have two aces in the same hand, one counts as 11 and the
	 * other counts 1
	 */
	...
	public void testInitial2CardsOfDealerAreVisible() {
		...
		assertEquals(true, dealer.dealerInitialCards());
	}
	...
	public void testInitial2CardsOfPlayerAreVisible() {
		...
		assertEquals(true, dealer.playerInitialCards());
	}
	...
	public void testPlayerScoringIsCorrect() {
		...
		assertEquals(dealer.getDealerHandValue(),
				dealer.dealersHand.get(0).getValue() + dealer.dealersHand.get(1).getValue());
	}
	...
	public void testDealerScoringIsCorrect() {
		...
		assertEquals(dealer.getPlayerHandValue(),
				dealer.playersHand.get(0).getValue() + dealer.playersHand.get(1).getValue());
	}

	// Test that a player can stand
	public void testIfPlayCanStand() {
		Deck deck = new Deck();
		Dealer dealer = new Dealer(deck);

		assertEquals(true, dealer.playerStand());
	}

	// Test that a player can repeatedly hit

	/*
	 * Test that the hand of a player is displayed at the end of the players turn
	 */
